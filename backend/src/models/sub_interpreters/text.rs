...
pub fn compare_texts(executor: &mut dyn BlockExecutor, block: &Block) -> Option<Value> {
    if let Some(inputs) = &block.inputs {
        let left = inputs.get("TEXT1").and_then(|i| get_string_input(executor, i)).unwrap_or_default();
        let right = inputs.get("TEXT2").and_then(|i| get_string_input(executor, i)).unwrap_or_default();
